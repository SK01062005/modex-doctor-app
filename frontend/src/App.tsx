...
const triageSymptom = (text: string) => {
  ...
  if (lower.includes('head') || lower.includes('migraine')) return 'Neurology';
  return '';
};
...
const Loader = () => (
  <div style={{ 
    ...
    backgroundSize: 'cover', backgroundPosition: 'center', zIndex: 9999
  }}>
    <div style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(44, 62, 80, 0.9)' }}></div>
    <div style={{ position: 'relative', zIndex: 2, textAlign: 'center' }}>
      {/* Heart Icon SVG */}
      <svg className="heartbeat" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#e74c3c" width="80px" height="80px">
      ...
      <div style={{ position: 'relative', zIndex: 2 }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #2c3e50 0%, #3498db 100%)', color: 'white', padding: '50px 20px', textAlign: 'center', boxShadow: '0 10px 30px rgba(0,0,0,0.1)' }}>
          ...
          <div style={{ position: 'relative', maxWidth: '600px', margin: '30px auto 0' }}>
            <input 
              type="text" 
              placeholder="Describe symptoms (e.g., 'chest pain')..." 
              value={symptom}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSymptom(e.target.value)}
              style={{ width: '100%', padding: '15px 25px', borderRadius: '50px', border: 'none', fontSize: '1.1rem', boxShadow: '0 5px 20px rgba(0,0,0,0.2)', boxSizing: 'border-box' }}
            />
        ...
        {/* Message Banner */}
        {msg && <div style={{ position: 'fixed', top: '20px', left: '50%', transform: 'translateX(-50%)', zIndex: 1000, padding: '15px 30px', borderRadius: '50px', fontWeight: 'bold', color: 'white', backgroundColor: msg.includes('✅') ? '#2ecc71' : '#e74c3c', boxShadow: '0 10px 30px rgba(0,0,0,0.3)' }}>{msg}</div>}
        ...
        <div className="fade-in" style={{ 
          ...
          {filteredSlots.map((slot) => (
            ...
            onMouseEnter={(e: React.MouseEvent<HTMLDivElement>) => { if(slot.status==='AVAILABLE') e.currentTarget.style.transform = 'translateY(-12px) scale(1.02)'; }}
            onMouseLeave={(e: React.MouseEvent<HTMLDivElement>) => { if(slot.status==='AVAILABLE') e.currentTarget.style.transform = 'translateY(0) scale(1)'; }}
            >
              {/* Image Area */}
